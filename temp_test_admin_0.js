...
async function findAdminMatches(id) {
  ...
  document.getElementById('matchModalClose')?.addEventListener('click', () => {
  ...
  try {
    ...
    if (mData.matches && mData.matches.length > 0) {
      mContent.innerHTML = mData.matches.map(m => {
        ...
        return `
                <div data-match-id="${m.id}" style="border:1px solid ${isHigh ? 'var(--amber)' : 'var(--border)'}; padding:12px; border-radius:8px; display:flex; gap:12px; align-items:center; background:var(--surface2);">
                  ...
                  <div style="flex:1;">
                    ...
                    <div style="display:flex; justify-content:space-between; align-items:center;">
                      ...
                      <div style="display:flex; gap:6px;">
                        ...
                        <button class="cancel-btn" style="padding:4px 10px; font-size:0.8rem; background:var(--muted);" onclick="this.closest('[data-match-id]').remove()">Ignore</button>
                      </div>
                    </div>
                  </div>
                </div>`;
